
# Welcome {#index -}

```{r, include = FALSE}
knitr::opts_chunk$set(fig.width=6, fig.height=3.5, fig.align="center")
```

<img src="figures/R.jpg" width="200" style="display: block; float:right; margin-left: 20px; margin-bottom: 20px;">

This is workshop material for the "Introduction to R" workshop given by the [Monash Genomics and Bioinformatics Platform](https://www.monash.edu/researchinfrastructure/mgbp). In the past it has been presented as part of the Data Fluency program at Monash. Our teaching style is based on the style of [The Carpentries](https://carpentries.org/).

* [Slideshow](slides.html)

* [PDF version for printing](r-intro-2.pdf)
* [ZIP of data files used in this workshop](r-intro.zip)

During the workshop we will be using Posit Cloud to use R over the web:

* [Posit Cloud](https://posit.cloud/)

You can also install R on your own computer. There are two things to download and install:

* [Download R](https://cran.rstudio.com/)
* [Download RStudio](https://www.rstudio.com/products/rstudio/download/)

R is the language itself. RStudio provides a convenient environment in which to use R, either on your local computer or on a server.


### Source code {-}

This book was created in R using [quarto](https://quarto.org/)!

* [GitHub page](https://github.com/MonashDataFluency/r-intro-2)

### Authors and copyright {-}

This course is developed for the Monash Genomics and Bioinformatics Platform by Paul Harrison.

```{r echo=F,fig.align="left"}
knitr::include_graphics("figures/CC-BY.png")
```

This work is licensed under a [CC BY-4: Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/). The attribution is "Monash Genomics and Bioinformatics Platform" if copying or modifying these notes.

Data files are derived from Gapminder, which has a CC BY-4 license. The attribution is "Free data from www.gapminder.org". The data is given here in a form designed to teach various points about the R language. Refer to the [Gapminder site](https://www.gapminder.org) for the original form of the data if using it for other uses.

```{r echo=F,out.width="100%",fig.align="center"}
knitr::include_graphics("figures/mgbp.png")
```
